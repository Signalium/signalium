...
```bash
npm install querius signalium
```

For React support:

```bash
npm install querius signalium react
```
...
```typescript
import { QueryClient, SyncQueryStore, MemoryPersistentStore, query, t } from 'querius';
...
```typescript
import { QueryClientContext } from 'querius';
...
```typescript
import { SyncQueryStore, MemoryPersistentStore } from 'querius';
...
```typescript
import { AsyncQueryStore } from 'querius/stores/async';
...
```typescript
import { NetworkManager } from 'querius';
