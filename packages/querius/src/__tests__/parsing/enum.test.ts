...
describe('t.enum.caseInsensitive', () => {
  describe('CaseInsensitiveSet class', () => {
    it('should create a CaseInsensitiveSet with string values', () => {
      ...
      expect((enumSet as any).size).toBe(3);
    });
    ...
    it('should allow same lowercase for non-string values', () => {
      ...
      expect((enumSet as any).size).toBe(5);
    });
  });
  ...
  describe('has() method', () => {
    it('should return true for exact matches', () => {
      ...
      expect((enumSet as any).has('Active')).toBe(true);
      expect((enumSet as any).has('Inactive')).toBe(true);
      expect((enumSet as any).has('Pending')).toBe(true);
    });
    ...
    it('should return false for non-matching values', () => {
      ...
      expect((enumSet as any).has('Unknown')).toBe(false);
      expect((enumSet as any).has('')).toBe(false);
      expect((enumSet as any).has('Activ')).toBe(false);
    });
    ...
    it('should perform exact match for numbers', () => {
      ...
      expect((enumSet as any).has(1)).toBe(true);
      expect((enumSet as any).has(2)).toBe(true);
      expect((enumSet as any).has(4)).toBe(false);
    });

    it('should perform exact match for booleans', () => {
      const enumSet = t.enum.caseInsensitive(true, false);

      expect((enumSet as any).has(true)).toBe(true);
      expect((enumSet as any).has(false)).toBe(true);
    });
  });
  ...
  describe('get() method', () => {
    it('should return canonical casing for string matches', () => {
      ...
      expect((enumSet as any).get('active')).toBe('Active');
      expect((enumSet as any).get('ACTIVE')).toBe('Active');
      expect((enumSet as any).get('inactive')).toBe('INACTIVE');
      expect((enumSet as any).get('PENDING')).toBe('pending');
    });

    it('should return undefined for non-matching strings', () => {
      const enumSet = t.enum.caseInsensitive('Active', 'Inactive') as unknown as CaseInsensitiveSet<string>;

      expect((enumSet as any).get('Unknown')).toBeUndefined();
      expect((enumSet as any).get('')).toBeUndefined();
    });

    it('should return the value for exact number matches', () => {
      const enumSet = t.enum.caseInsensitive(1, 2, 3) as unknown as CaseInsensitiveSet<number>;

      expect((enumSet as any).get(1)).toBe(1);
      expect((enumSet as any).get(2)).toBe(2);
      expect((enumSet as any).get(4)).toBeUndefined();
    });
    ...
    it('should return the value for exact boolean matches', () => {
      ...
      expect((enumSet as any).get(true)).toBe(true);
      expect((enumSet as any).get(false)).toBeUndefined();
    });
  });
  ...
  describe('iteration', () => {
    it('should iterate over canonical values', () => {
      const enumSet = t.enum.caseInsensitive('Active', 'Inactive', 'Pending') as unknown as CaseInsensitiveSet<string>;
      const values = Array.from(enumSet as any);
  ...
  describe('Direct parseValue', () => {
    describe('basic parsing', () => {
      it('should parse exact match and return as-is', () => {
        const enumDef = t.enum.caseInsensitive('Active', 'Inactive', 'Pending') as unknown as CaseInsensitiveSet<string>;
