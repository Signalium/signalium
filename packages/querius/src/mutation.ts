...
export class Mutation {
  ...
  request!: ResponseTypeDef;
  response!: ResponseTypeDef;
  optimisticUpdates?: boolean;
  cache?: MutationCacheOptions;
}
...
const MUTATION_FN_MAP = new Map<Function, () => MutationDefinition<any, any>>();

// eslint-disable-next-line @typescript-eslint/no-unsafe-function-type
export const mutationKeyFor = (fn: Function): string => {
  const mutationDef = MUTATION_FN_MAP.get(fn);
...
export function getMutation<T extends Mutation>(
  ...
  MutationResultValue<Readonly<ExtractMutationResponse<T>>>,
  [ExtractMutationRequest<T>]
> {
