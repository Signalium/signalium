...
export class ValidatorDef<T> {
  ...
  private _shape: InternalTypeDef | InternalObjectShape | UnionTypeDefs | (() => InternalObjectShape) | ComplexTypeDef[] | undefined;
  ...
  constructor(
    ...
    mask: Mask,
    shape: InternalTypeDef | InternalObjectShape | UnionTypeDefs | (() => InternalObjectShape) | ComplexTypeDef[] | undefined,
...
export function defineArray<T extends TypeDef>(shape: T): TypeDef<ExtractType<T>[]> {
  return new ValidatorDef(ComplexTypeDefKind.ARRAY, Mask.ARRAY, shape as unknown as InternalTypeDef) as unknown as TypeDef<ExtractType<T>[]>;
}

export function defineRecord<T extends TypeDef>(shape: T): TypeDef<Record<string, ExtractType<T>>> {
  return new ValidatorDef(ComplexTypeDefKind.RECORD, Mask.RECORD | Mask.OBJECT, shape as unknown as InternalTypeDef) as unknown as TypeDef<Record<string, ExtractType<T>>>;
}

export function defineParseResult<T extends TypeDef>(innerType: T): TypeDef<import('./types.js').ParseResult<ExtractType<T>>> {
...
export function defineObject<T extends Record<string, TypeDef>>(shape: T): TypeDef<Prettify<ExtractTypesFromShape<T>>> {
  return new ValidatorDef(ComplexTypeDefKind.OBJECT, Mask.OBJECT, shape as unknown as InternalObjectShape) as unknown as TypeDef<Prettify<ExtractTypesFromShape<T>>>;
}
...
function defineNullish<T extends TypeDef>(type: T): TypeDef<ExtractType<T> | undefined | null> {
  ...
  if (t instanceof Set) {
    return defineUnion(type, Mask.UNDEFINED as unknown as TypeDef<undefined>, Mask.NULL as unknown as TypeDef<null>) as unknown as R;
  }
