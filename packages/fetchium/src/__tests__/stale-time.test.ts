  ...
  describe('Fresh Data', () => {
    it('should not refetch when data is fresh (within staleTime)', async () => {
      ...
      await testWithClient(client, async () => {
        ...
        expect(relay2.value!).toMatchObject({ value: 'first' });
    ...
    it('should use fresh data from disk cache without refetch', async () => {
      ...
      await testWithClient(client2, async () => {
        ...
        relay.value;
        await sleep(10);
        expect(relay.value!).toMatchObject({ data: 42 });
  ...
  describe('Stale Data', () => {
    it('should serve stale data immediately while refetching in background', async () => {
      ...
      await testWithClient(client, async () => {
        ...
        expect(relay2.value!).toMatchObject({ count: 1 });

        // Wait for background refetch to complete
        await sleep(100);

        // Should now have fresh data
        expect(relay2.value!).toMatchObject({ count: 2 });
    ...
    it('should refetch stale data from disk cache', async () => {
      ...
      await testWithClient(client2, async () => {
        ...
        relay.value;
        await sleep(10);
        expect(relay.value!).toMatchObject({ version: 1 });

        // Should trigger background refetch
        await sleep(100);

        expect(relay.value!).toMatchObject({ version: 2 });
        expect(mockFetch.calls).toHaveLength(1);
      });
    });
    ...
    it('should handle no staleTime (always refetch)', async () => {
      ...
      await testWithClient(client, async () => {
        ...
        expect(relay2.value!).toMatchObject({ value: 'second' });
        expect(mockFetch.calls).toHaveLength(2);
      });
    });
  });
  ...
  describe('Edge Cases', () => {
    it('should handle staleTime of 0 (always stale)', async () => {
      ...
      await testWithClient(client, async () => {
        ...
        relay2.value;
        await sleep(10);
        expect(relay2.value!).toMatchObject({ n: 1 });

        await sleep(100);
        expect(relay2.value!).toMatchObject({ n: 2 });
      });
    });
    ...
    it('should handle very long staleTime', async () => {
      ...
      try {
        ...
        await testWithClient(client, async () => {
          ...
          await relay1;
          expect(relay1.value!).toMatchObject({ data: 'cached' });
          expect(mockFetch.calls).toHaveLength(1);
        });
        ...
        await testWithClient(client, async () => {
          ...
          expect(relay2.value!).toMatchObject({ data: 'cached' });
          expect(mockFetch.calls).toHaveLength(0);
        });
        ...
        await testWithClient(client, async () => {
          ...
          expect(relay3.value!).toMatchObject({ data: 'cached' });
          expect(mockFetch.calls).toHaveLength(0);
        });
        ...
        await testWithClient(client, async () => {
          ...
          expect(relay4.value!).toMatchObject({ data: 'cached' });

          // Wait for background refetch to complete
          await vi.advanceTimersByTimeAsync(100);

          // Should now have fresh data
          expect(relay4.value!).toMatchObject({ data: 'fresh-after-hour' });
          expect(mockFetch.calls).toHaveLength(1);
        });
      } finally {
        vi.runAllTimers();
    ...
    it('should handle concurrent access to stale data', async () => {
      ...
      await testWithClient(client, async () => {
        ...
        relay2.value;
        await sleep(10);
        expect(relay2.value!).toMatchObject({ id: 1 });

        // Wait for refetch
        await sleep(100);
        expect(relay2.value!).toMatchObject({ id: 2 });
