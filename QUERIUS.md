# Querius — detailed architecture

Data-fetching and entity management layer built on signalium. Lives in `packages/querius/`.

## Entry points

| Import path | Source |
|---|---|
| `querius` | `src/index.ts` |
| `querius/react` | `src/react/index.ts` (exports `useQuery`) |
| `querius/stores/sync` | `src/stores/sync.ts` (sync query store) |
| `querius/stores/async` | `src/stores/async.ts` (async/IndexedDB query store) |
