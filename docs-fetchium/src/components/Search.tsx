...
function HighlightQuery({ text, query }: { text: string; query: string }) {
  return (
    <Highlighter
      highlightClassName="group-aria-selected:underline bg-transparent text-secondary-400"
      searchWords={[query]}
      autoEscape={true}
...
function SearchResult({
  ...
  let sectionTitle = findNavigationItem(result.url.split('#')[0])?.title;
    ...
    >
      ...
      {hierarchy.length > 0 && (
        <div
          id={`${id}-hierarchy`}
          aria-hidden="true"
          className="mt-0.5 truncate text-xs whitespace-nowrap text-slate-400"
        >
...
>(function SearchInput({ autocomplete, autocompleteState, onClose }, inputRef) {
  ...
  return (
    <div className="group relative flex h-12">
      <SearchIcon className="pointer-events-none absolute top-0 left-4 h-full w-5 fill-slate-500" />
      <input
        ref={inputRef}
        data-autofocus
        className={clsx(
          'flex-auto appearance-none bg-transparent pl-12 text-white outline-hidden placeholder:text-slate-400 focus:w-full focus:flex-none sm:text-sm [&::-webkit-search-cancel-button]:hidden [&::-webkit-search-decoration]:hidden [&::-webkit-search-results-button]:hidden [&::-webkit-search-results-decoration]:hidden',
          ...
          <DialogPanel className="mx-auto transform-gpu overflow-hidden rounded-xl bg-primary-900 shadow-xl ring-1 ring-primary-400/50 sm:max-w-xl">
            <div {...autocomplete.getRootProps({})}>
              ...
              >
                ...
                <div
                  ref={panelRef}
                  className="border-t border-primary-400/10 bg-primary-900 px-2 py-3 empty:hidden"
...
export function Search() {
  ...
  return (
    <>
      <button
        type="button"
        className="group flex h-6 w-6 items-center justify-center transition-all sm:justify-start md:h-auto md:w-80 md:flex-none md:rounded-lg md:bg-primary-900/60 md:py-2.5 md:pr-3.5 md:pl-4 md:text-sm md:ring-1 md:ring-primary-400/20 md:ring-inset md:hover:bg-primary-800/40 md:hover:ring-primary-400/50 lg:w-96"
        {...buttonProps}
      >
        <SearchIcon className="h-5 w-5 flex-none fill-primary-300/30 transition-all group-hover:fill-primary-300/60" />
        <span className="sr-only md:not-sr-only md:ml-2 md:text-primary-400">
          Search docs
        </span>
        {modifierKey && (
